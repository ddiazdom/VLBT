...
template<>
struct custom_wt_rlmn_trait<byte_alphabet_tag> {
    enum {width = 8};
...
class custom_wt_rlmn
{
    public:
        ...
        typedef random_access_const_iterator<custom_wt_rlmn> const_iterator;
        ...
        enum { width = custom_wt_rlmn_trait<alphabet_category>::width };
        typedef typename custom_wt_rlmn_trait<alphabet_category>::C_type
        C_type;
        typedef typename custom_wt_rlmn_trait<alphabet_category>::C_bf_rank_type
        C_bf_rank_type;
    ...
    public:
        ...
        explicit custom_wt_rlmn(std::string& bwt_file) {
            ...
            m_C_bf_rank = custom_wt_rlmn_trait<alphabet_category>::init_C_bf_rank(m_C, size);
         ...
         *                   the wavelet tree should be build.
         */
        custom_wt_rlmn(int_vector_buffer<width>& text_buf, size_type size):m_size(size) {
            std::string temp_file = text_buf.filename() +
                                    + "_custom_wt_rlmn_" + util::to_string(util::pid())
        ...
        //! Copy constructor
        custom_wt_rlmn(const custom_wt_rlmn& wt) {
            copy(wt);
        }
        ...
        //! Assignment operator
        custom_wt_rlmn& operator=(const custom_wt_rlmn& wt) {
            if (this != &wt) {
                copy(wt);
            }
            return *this;
        ...
        custom_wt_rlmn& operator=(custom_wt_rlmn&& wt) {
            if (this != &wt) {
        ...
        [[nodiscard]] std::pair<uint8_t, size_t> lf(size_type i) const {
            ...
            size_t next = m_C[res.second] + res.first;
