...
void test_access(competitor_bwt& their_bwt, fb_bwt& my_bwt){
    ...
    std::cout<<"their average time: "<<double(their_time)/double(samp_size)<<" nano seconds\n"<<std::endl;
}
...
void test_rank(competitor_bwt& their_bwt, fb_bwt& my_bwt){
    ...
    for(size_t i=0;i<samp_size;i++){
        ...
        uint8_t s = rand() % my_bwt.alphabet;
...
void test_select(competitor_bwt& their_bwt, fb_bwt& my_bwt){
    ...
    //get the max rank of each symbol to avoid failed asserts
    size_t rank_answers[16] ={0};
    for(size_t i=0;i<my_bwt.alphabet;i++){
...
int main(int argc,  char** argv) {
    ...
    if(argc!=2){
        std::cout<<"usage: ./test_bwts prefix"<<std::endl;
        exit(1);
    }
    ...
    test_interval_symbols(input_file);
    test_select(input_file);
    test_inverse_select(input_file);
    test_access(input_file);
    test_rank(input_file);
