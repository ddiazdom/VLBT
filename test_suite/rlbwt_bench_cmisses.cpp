//
// Created by Diaz, Diego on 26.11.2025.
//
//
...
std::vector<std::pair<uint64_t, uint8_t>> compute_random_rank_queries(const dt_type& dt, uint64_t n_samps) {
    ...
    const size_t alphabet_size = dt.alphabet();
...
template<class dt_type>
void bench_int(const std::string& input_index, const size_t n_samp, const std::string& pat_file) {
    dt_type bwt_dt;
    sdsl::load_from_file(bwt_dt, input_index);
    benchmark_access(bwt_dt, std::min<size_t>(n_samp, bwt_dt.size()));
    benchmark_rank(bwt_dt, std::min<size_t>(n_samp, bwt_dt.size()));
    benchmark_count(bwt_dt, pat_file);
}

int main(int argc, char** argv) {

    if(argc!=5){
        std::cout<<"usage: ./rlbwt_bench_cmisses <input_dt> <dt_type> <n_samples> <pat_file>"<<std::endl;
        exit(1);
    }

    const auto input_index = std::string(argv[1]);
    char *pend;
    long int dt_type = strtol(argv[2], &pend, 10);
    long int n_samp = strtol(argv[3], &pend, 10);
    const auto pat_file = std::string(argv[4]);

    switch (dt_type) {
        case 0:
            bench_int<simple_rlbwt<sdsl::wt_rlmn<>>>(input_index, n_samp, pat_file);
            break;
        case 1:
            bench_int<simple_rlbwt<wt_fbb<>>>(input_index, n_samp, pat_file);
            break;
        default:
            exit(1);
    }
